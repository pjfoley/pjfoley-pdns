...
#   Example: template => 'site/pdns/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/pdns/my.conf',
#
# [*ensure*]
...
define pdns::backend::bind::conf (
  ...
  include pdns::backend::bind

  $manage_path    = pickx($path, "${pdns::backend::bind::path_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $pdns::backend::bind::config_file_mode)
  $manage_owner   = pickx($owner, $pdns::backend::bind::config_file_owner)
  $manage_group   = pickx($group, $pdns::backend::bind::config_file_group)
  $manage_require = pickx($config_file_require, $pdns::backend::bind::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $pdns::backend::bind::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "pdns_conf_${name}":
    ensure  => $ensure,
    source  => $source,
